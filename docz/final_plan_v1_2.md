# 🎯 CADAS APP — FINAL PLAN v1.2
## Rujukan Tunggal & Final untuk Developer — Fase Linear dari Nol hingga Rilis

**Tanggal disusun:** 8 September 2026
**Versi:** 1.2 (revisi dari v1.1 — tambah Sprint D: Xendit + Admin Dashboard + Referrer System)
...
FASE 4  Placement Test End-to-End
   ...
   ├──────────────┐
   ▼              ▼
FASE 5          FASE 6  Billing Manual (selesai) + Xendit (Sprint D)
(Practice Loop)    │
   │               ▼
...
(Parent)       (Guru)         Sprint D: Xendit + Admin Dashboard + Referrer
   │              │              │
   └──────┬───────┘              │
          ▼                      │
...
# FASE 12 — SPRINT D: XENDIT + ADMIN DASHBOARD + REFERRER SYSTEM

### 🎯 Tujuan
Billing otomatis via Xendit, admin punya dashboard operasional lengkap, referrer
...
- Xendit documentation: Invoice API + Webhook
...
### ✅ D.1 — DB Schema Migration (`012_xendit_referrer_dashboard.sql`)
...
CREATE INDEX idx_xendit_invoices_student ON xendit_invoices(student_id);
CREATE INDEX idx_xendit_invoices_status ON xendit_invoices(status);
```
...
```javascript
// Fungsi utama:
createInvoice({ studentId, productType, levelFrom, levelTo, amountIdr, referrerCode })
  → POST https://api.xendit.co/v2/invoices
  → Simpan ke xendit_invoices
  → Return { invoiceUrl, xenditInvoiceId }

verifyWebhookSignature(req)
  → Validasi header x-callback-token === process.env.XENDIT_CALLBACK_TOKEN

handlePaidCallback(xenditInvoiceId)
  → Update xendit_invoices.status = 'paid', paid_at = NOW()
...
POST /api/payment/create-invoice
  ...
  → createInvoice() → return { invoice_url, xendit_invoice_id, amount_idr, expires_at }

POST /api/webhooks/xendit
  Auth: x-callback-token header
  Body: Xendit webhook payload
  → verifyWebhookSignature() → handlePaidCallback()
  → Return 200 (Xendit butuh 200 cepat, proses async)
```

**Environment variables baru di `.env`:**
```
XENDIT_SECRET_KEY=xnd_production_...
XENDIT_CALLBACK_TOKEN=...   # dari Xendit dashboard
XENDIT_SUCCESS_REDIRECT_URL=https://cadas.app/payment/success
XENDIT_FAILURE_REDIRECT_URL=https://cadas.app/payment/failed
```

**⚠️ Catatan penting:**
- Xendit test mode gunakan `XENDIT_SECRET_KEY=xnd_development_...`
- Webhook butuh URL publik — untuk dev lokal pakai ngrok atau Xendit test payload
- Manual fallback `/api/admin/billing/activate` TETAP ADA dan tidak berubah

**🧪 Backtest D.2:**
```
POST /api/payment/create-invoice (valid) → invoice_url Xendit valid
POST /api/payment/create-invoice (parent bukan pemilik student) → 403
POST /api/webhooks/xendit (callback-token salah) → 401
POST /api/webhooks/xendit (simulasi paid) → level ter-activate
  → xendit_invoices.status = 'paid'
...
GET /api/admin/dashboard/summary
  → {
      ...
      payments: { pending_xendit, confirmed_manual, failed_expired },
...
GET /api/admin/dashboard/payments
  Query: ?status=all|pending|paid|expired&from=YYYY-MM-DD&to=YYYY-MM-DD&page=1&limit=20
  → Gabungan xendit_invoices + payment_records, sorted by created_at DESC
  → Setiap row: { source: 'xendit'|'manual', student_name, product_type,
...
- [ ] D.1: Migration 012 berjalan tanpa error, semua tabel/kolom ada
- [ ] D.2: Xendit invoice create + webhook handler berjalan (test mode)
...
- [ ] Backtest end-to-end: bayar via Xendit → level aktif → komisi tercatat → tampil di referrer dashboard

### ⚠️ Catatan Fase
```
Status        : PLANNING — belum dimulai
Prerequisite  : Sprint A/B/C selesai (✅), git commit (❌ belum)
Xendit key    : Belum ada — daftar di dashboard.xendit.co, gunakan test mode dulu
...
- ADMIN_SECRET kuat, XENDIT_SECRET_KEY production

**14.4 Ops:**
- Backup DB otomatis (daily)
- Error alerting (Sentry atau minimal email)
- Xendit webhook URL pointing ke server production (bukan ngrok)

### 🏁 Kriteria selesai
- [ ] APK bisa diinstall dari `cadas.app/download`
- [ ] Xendit webhook production aktif
...
| 2 | Bulk purchase kelompok (min 10 siswa) — alur Xendit-nya? Satu invoice atau batch? | Sprint D.2 |
| 3 | Gap 1.4: 15/15 explanation_variants sudah lolos audit? | Fase 1 |
| 4 | Xendit test key sudah ada? Atau perlu daftar dulu? | Sprint D.2 |
