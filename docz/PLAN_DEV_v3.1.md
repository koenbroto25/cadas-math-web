...
**Status:** v3.1 — menambahkan sistem referral siswa (peer referral), token referral tersembunyi
(tanpa kode referral visible di URL), dan sistem tier komisi berbasis performa (Model B).
...
**Perubahan dari v3.0:** Siswa (usia SMP ke atas, terverifikasi oleh orang tua) dapat menjadi
referrer dengan komisi khusus — memperkenalkan konsep "belajar sekaligus menghasilkan".
Sistem referral beralih dari kode visible ke token acak tersembunyi (cadas.app/d/Xk9mP).
...
| 19 | `referrers.type` tambah nilai `student` — siswa (SMP ke atas) bisa jadi referrer dengan consent orang tua | Siswa yang sudah pakai app punya kredibilitas alami untuk rekomendasikan ke teman; edukasi finansial sejak dini |
| 20 | Sistem token referral tersembunyi (`cadas.app/d/Xk9mP`) — token acak, tidak mengekspos identitas referrer | Format seragam, tidak kentara sebagai link referral, sesuai UU PDP 2022 dengan satu kalimat di Privacy Policy |
| 21 | Tabel `download_clicks` untuk IP-based attribution — cocokkan download ke register dalam window 2 jam | Sederhana, tanpa SDK pihak ketiga, cukup akurat untuk skala awal Indonesia |
...
CREATE TABLE referrers (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  type VARCHAR(20) NOT NULL,                -- DIPERBARUI v3.1: 'teacher_private' | 'teacher_school'
                                            -- | 'affiliate' | 'parent' | 'student' | 'other'
                                            --
                                            ...
                                            --
                                            -- parent: orang tua siswa — kanal marketing
                                            --   organik dari komunitas sekolah.
                                            --
                                            -- student: siswa SMP ke atas yang sudah verified
                                            --   oleh orang tuanya (parent_consent_at tidak NULL).
                                            --   Dapat komisi lebih rendah — edukasi finansial
                                            --   sejak dini, bukan income utama.
  reference_id UUID,                        -- teachers.id, students.id, parents.id, atau NULL
  token VARCHAR(20) UNIQUE NOT NULL,        -- DIPERBARUI v3.1: token acak (mis: 'Xk9mP'),
                                            -- menggantikan 'code' yang dulu visible.
                                            -- generate via: nanoid(8) atau crypto.randomBytes
  commission_model VARCHAR(30),             -- NULL untuk teacher_school
                                            -- 'recurring_percentage' untuk teacher_private, parent
                                            -- 'one_time_bonus' untuk affiliate/other
                                            -- 'student_recurring' untuk student (rate lebih rendah)
  commission_rate NUMERIC(5,2),             -- 0.00 untuk teacher_school
                                            -- mis: 10.00 untuk 10% recurring (teacher_private)
                                            -- mis: 5.00 untuk 5% (parent)
                                            -- mis: 3.00 untuk 3% (student)
  -- Tier komisi berbasis performa (Model B) — berlaku untuk semua tipe kecuali teacher_school
  total_active_referrals INT DEFAULT 0,     -- jumlah siswa aktif (is_premium) yang dirujuk
  tier VARCHAR(20) DEFAULT 'mitra',         -- 'mitra' (0-9) | 'andalan' (10-29) | 'utama' (30+)
  -- Khusus untuk tipe 'student'
  parent_consent_at TIMESTAMP,             -- NULL = belum ada consent orang tua,
                                            -- sistem TIDAK boleh proses komisi jika masih NULL
  min_age_verified BOOLEAN DEFAULT false,   -- true jika admin konfirmasi usia >= 13 tahun (SMP)
  created_at TIMESTAMP DEFAULT NOW()
);

-- Index untuk lookup token (endpoint GET /d/:token)
CREATE INDEX idx_referrers_token ON referrers(token);

-- Tabel baru v3.1: tracking klik download sebelum register
-- Digunakan untuk IP-based attribution (jendela 2 jam)
CREATE TABLE download_clicks (
  ...
  attributed BOOLEAN DEFAULT false          -- jadi true setelah di-match ke siswa yang register
);

-- Index untuk lookup IP (dipakai saat register siswa)
CREATE INDEX idx_download_clicks_ip ON download_clicks(ip_address, clicked_at DESC);
...
StudentStack        → PlacementScreen (wajib), HomeScreen, PracticeScreen,
                      ...
                      AjakTemanScreen (BARU v3.1 — kondisional, syarat lihat §12.3),
...
-- Referral (DIPERBARUI v3.1)
GET    /d/:token                     — catat download_click, redirect ke halaman download
POST   /api/referrals                — daftarkan referrer baru (admin atau self-register)
GET    /api/referrals/stats          — stats referrer berdasarkan JWT (untuk dashboard referrer)
POST   /api/referrals/student/request  — siswa request jadi referrer (butuh parent consent)
POST   /api/referrals/student/consent  — parent setujui consent referral siswa
```
...
- [ ] `TeacherAuthScreen.jsx` — dua sub-alur berdasarkan `teacher_type`:
  ...
  - `private`: verifikasi lebih ringan (bisa email biasa + deskripsi singkat). **Setelah verified → dapat link token + dashboard komisi.**
- [ ] `ClassroomDashboardScreen.jsx`
- [ ] `AssignmentsScreen.jsx`
- [ ] `ParentMessagesScreen.jsx`
- [ ] `AjakTemanScreen.jsx` (BARU v3.1) — hanya muncul di StudentStack jika siswa memenuhi syarat:
  - Tampilkan jumlah teman bergabung (tanpa identitas)
  - Tombol share link token (WhatsApp / copy)
  - Estimasi kredit/komisi bulan ini
  - Framing edukatif, bukan promosi income

## 12.3 Referral System (Diperbarui v3.1 — Token Tersembunyi + Siswa sebagai Referrer)

### Sistem Token (menggantikan kode visible lama)

```
Setiap referrer mendapat link unik dengan token acak:
  cadas.app/d/Xk9mP   ← token 8 karakter, generate dari crypto.randomBytes
  cadas.app/d/7tRnQ
  cadas.app/d/mK2sL

Semua link terlihat identik formatnya — tidak ada yang tahu itu link referral.
Server yang tahu bahwa Xk9mP → referrer tertentu (ada di database, bukan di URL).

Endpoint baru di Sprint 7:
  GET /d/:token
    → Cari referrer berdasarkan token
    → Catat baris baru di download_clicks (ip_address, user_agent, clicked_at)
    → Redirect ke halaman download APK / landing page

Saat siswa register (POST /api/auth/register):
    → Ambil IP dari request
    → Cari download_clicks di IP yang sama dalam 2 jam terakhir, attributed = false
    → Ketemu → set students.referred_by = referrer_id
                set download_clicks.attributed = true
    → Tidak ketemu → referred_by = NULL

Catatan di Privacy Policy (UU PDP 2022):
  "Kami melacak asal unduhan untuk menghitung komisi mitra pendidik kami."
  — Tidak ada kewajiban banner di dalam app untuk tracking jenis ini.
```

### Tier Komisi Berbasis Performa (Model B — berlaku semua tipe kecuali teacher_school)

```
Tier dihitung dari total_active_referrals (siswa premium yang dirujuk, masih aktif):

  Mitra   : 0–9 siswa aktif   → commission_rate berlaku sesuai tipe
  Andalan : 10–29 siswa aktif → +2% dari rate dasar tipe tersebut
  Utama   : 30+ siswa aktif   → +5% dari rate dasar (setara tier tertinggi)

Rate dasar per tipe:
  teacher_private  → 10% recurring per bulan
  parent           →  5% recurring per bulan
  student          →  3% recurring per bulan (lebih rendah, sesuai konteks usia)
  affiliate/other  → one-time bonus (nominal ditetapkan per kesepakatan)

Contoh: guru les dengan 35 siswa aktif → tier Utama → efektif 15% per bulan
Contoh: siswa SMP dengan 12 teman aktif → tier Andalan → efektif 5% per bulan

Update tier dilakukan otomatis oleh backend setiap kali payment_records diisi
atau siswa churn (berhenti langganan).
```

### Referral Guru Sekolah

```
Guru sekolah (teacher_school):
  - Dapat link token HANYA untuk referral ke luar kelasnya
    (misal: rekomendasikan ke guru lain di sekolah lain, atau grup WA orang tua
     di luar institusinya)
  - Untuk siswa di kelasnya sendiri: TIDAK dapat komisi — ini sengaja untuk
    hindari konflik kepentingan institusional
  - commission_rate = 0.00 di tabel referrers
```

### Referral Guru Les / Tutor Privat

```
Guru les / tutor privat (teacher_private):
  - Dapat link token untuk semua konversi yang direferensikan
  - commission_model = 'recurring_percentage', commission_rate = 10.00 (default)
  - Komisi dihitung otomatis saat admin input payment_records dengan referrer_id
  - Fase MVP: rekap komisi dibuat manual bulanan, transfer via admin
  - Fase 2: payout otomatis via Xendit Disbursement
```

### Referral Siswa (BARU v3.1)

```
Siswa sebagai referrer — konsep "belajar sekaligus menghasilkan":

Syarat pendaftaran sebagai referrer:
  1. Usia minimal 13 tahun (SMP) — admin verifikasi, min_age_verified = true
  2. Consent eksplisit dari orang tua — parent_consent_at ter-set (lewat ParentDashboard)
  3. Siswa sudah is_premium (harus sudah pakai produk, bukan sekedar daftar)

Alur pendaftaran siswa sebagai referrer:
  Siswa buka menu "Ajak Teman" di StudentStack
    → Tampilkan penjelasan singkat: cara kerja, berapa komisi, kapan bisa cair
    → Tombol "Minta Izin Orang Tua" → kirim notifikasi / WA ke parent
  
  Orang tua buka ParentDashboard → ada request consent
    → Orang tua setujui → parent_consent_at ter-set di referrers
    → Link token otomatis aktif, bisa dibagikan siswa

Aturan komisi siswa:
  - commission_rate = 3.00 (3% recurring) — rate paling rendah
  - Komisi bukan diberikan tunai langsung ke siswa (anak di bawah umur)
  - Opsi penyaluran komisi (orang tua pilih satu):
      a. Kredit perpanjangan premium anak itu sendiri (paling sederhana)
      b. Transfer ke rekening orang tua (butuh data rekening di profil parent)
  - Batas maksimal komisi aktif: 50 konversi / siswa
    (mencegah siswa jadi "agen MLM muda" secara tidak sehat)

Batasan penting:
  - Siswa TIDAK bisa lihat identitas teman yang mendaftar lewat linknya
    (hanya jumlah: "3 teman sudah bergabung lewat linkmu")
  - Tidak ada leaderboard referral antar siswa — ini bukan kompetisi
  - Fitur "Ajak Teman" tidak menonjol di UI — tersedia tapi tidak dipush
    (jangan sampai motivasi belajar siswa bergeser ke motivasi komisi)

UI di StudentStack:
  Screen "Ajak Teman" (AjakTemanScreen.jsx, BARU):
    - Hanya muncul jika siswa is_premium + min_age_verified + parent_consent_at tidak NULL
    - Tampilkan: jumlah teman yang bergabung, estimasi kredit/komisi bulan ini
    - Tombol share link (WhatsApp, copy link)
    - Framing: "Bantu teman belajar matematika — kamu juga dapat bonus!"
      bukan "Hasilkan uang dengan referral"
```
...
- [ ] Link token guru les menghitung komisi dengan benar saat payment_records diisi
...
- [ ] `GET /d/:token` catat download_clicks dengan benar dan redirect ke halaman download
- [ ] IP attribution berjalan: siswa yang register dalam 2 jam setelah klik link ter-link ke referrer
- [ ] Siswa hanya bisa jadi referrer setelah: is_premium + min_age_verified + parent_consent_at tidak NULL
- [ ] `AjakTemanScreen` tidak muncul untuk siswa yang belum memenuhi syarat
- [ ] Siswa tidak bisa lihat identitas teman — hanya jumlah
- [ ] Pilihan penyaluran komisi siswa (kredit atau transfer ke orang tua) berfungsi
- [ ] Tier komisi (Mitra/Andalan/Utama) ter-update otomatis saat payment_records diisi
...
[ ] Guru les: link token ada dan komisi dihitung dengan benar
[ ] IP attribution berfungsi: klik link → register dalam 2 jam → referred_by ter-set
[ ] AjakTemanScreen hanya muncul untuk siswa is_premium + min_age_verified + parent_consent_at
[ ] Siswa tidak bisa lihat identitas teman lewat linknya, hanya jumlah total
[ ] Komisi siswa tidak bisa cair tanpa consent orang tua aktif
...
cadas-app v3.1: **platform 3-role penuh** dengan backend sendiri, model freemium berbasis level
...
Sistem referral menggunakan token acak tersembunyi (`cadas.app/d/Xk9mP`) — tidak ada kode
referral visible di URL. Attribution via IP fingerprinting (jendela 2 jam). Empat tipe referrer:
guru les (10%), orang tua (5%), siswa SMP ke atas dengan consent orang tua (3%), dan
affiliate/other (one-time bonus). Komisi siswa disalurkan sebagai kredit premium atau ke
rekening orang tua — bukan langsung ke siswa. Fitur "Ajak Teman" tersedia tapi tidak dipush
agar motivasi belajar siswa tetap utama.
